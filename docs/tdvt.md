...
Tableau provides an automated testing tool called the Tableau Datasource Verification Tool, or **TDVT**, to test Tableau connectivity to a database and the correctness of queries against the database. TDVT runs tests that range from simple expressions to complex SQL.
...
TDVT a test framework that uses the `tabquerytool` executable included with Tableau Desktop to run queries against tables in a datasource. For each test suite, TDVT calls the `tabquerytool` executable to run the queries, and then compares the results to expected files generated by our engineering team.

**Inputs** are a TDS file describing a table in your database, and a TDVT test file that contains either **logical query** or an **expression test(s)**.
...
Outputs for TDVT are data returned from the database after executing the query.
...
In this section, we'll use Postgres as an example to describe the setup and configuration steps you can use your own data source.

TDVT tests for these things:
...
1. Be sure your system includes these prerequisites:
    ...
    - Tableau Desktop installed and licensed.
    - Python 3.9+ (See: [Windows releases](https://www.python.org/downloads/windows/) / [Mac releases](https://www.python.org/downloads/mac-osx/))
      - `pip` (comes with the Python installation) On Windows, be sure to enable the "install environmental variables" option.
2. Clone the [Connector Plugin SDK](https://github.com/tableau/connector-plugin-sdk/) and install TDVT.
...
3. Install TDVT
   ...
   or on Windows:
    ...
    Install TDVT in your virtual environment:
        ```
        $ (tdvt-venv) cd connector-plugin-sdk/tdvt
        $ (tdvt-venv) python -m pip install -e .
        ```
    You can then verify TDVT installed by running `pip list`:
        ```
         $ pip list
         ...
         pip        23.2.1
         setuptools 65.5.1
         tdvt       2.7.1   /path/to/connector-plugin-sdk/tdvt/tdvt
         wheel      0.38.4
        ```

4. Extract and then load the [TestV1 dataset](https://github.com/tableau/connector-plugin-sdk/tree/master/tests/datasets/TestV1) into your database.
__Notes:__
    - See the [Postgres Example](https://github.com/tableau/connector-plugin-sdk/tree/master/tests/datasets/TestV1/postgres/README.md) for an example of loading data into a database
...
TDVT checks the current working directory for the test configuration files that set up test suites for your data source.
    - Run these steps to set up TDVT with a sample data source:
        1. Create a new directory, for example: `tdvt_workspace/`
...
`python -m tdvt.tdvt list-logical-configs`
   ...
   Search the output of the command for something that matches "[dbo].[Calcs]". For example:
   ```
   Name = dbo
   Calcs = [dbo].[Calcs]
   Staples = [dbo].[Staples]
