...
| value-type | Dictates the default validation rule and the UI widget | No | Allowed Values: UI Widget Type <br> `string`: text field <br> `textbox`: text area <br>  `option`: drop-down  <br> `boolean`: checkbox <br> `file`: file picker | In the 2020.2 release `file` is not supported. |
