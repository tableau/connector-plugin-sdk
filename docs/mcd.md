...
| placeholder | Placeholder text displayed as a help message when the fild has no user input | Yes | | |
| value-type | Dictates the default validation rule and the UI widget | No | Allowed Values: UI Widget Type <br> `string`: text field <br> `option`: drop-down <br> `boolean`: checkbox <br> `file`: file picker  <br> `textbox`: multiline text box | In the 2020.2 release `file` is not supported. |
