...
CAP_JDBC_METADATA_BIND_TIMESTAMPTZ_TO_DATETIME | Set to 'yes' to ensure that JDBC TIMESTAMP_WITH_TIMEZONE and TIME_WITH_TIMEZONE types are treated as Date & Time fields in Tableau. | &ndash; | &ndash;
...
CAP_JDBC_INCLUDE_CATALOG_NAME | Set 'yes' to include the catalog (database) name in the fully qualified table name. | &ndash; | &ndash;
...
CAP_ODBC_INCLUDE_CATALOG_NAME | Set 'yes' to include the catalog (database) name in the fully qualified table name. | &ndash; | &ndash;
...
CAP_EXTRACT_ONLY_FEDERATED | Set to 'yes' to enable extracted data to be joined with other data sources (i.e. cross-database joins). Available in Tableau 2023.2 and newer. | &ndash; | &ndash;
